...
// ─── Auth helpers (mirror existing pattern in routes/auth.ts & patient.ts) ──
...
function decodeBearerId(authHeader: string | undefined): string | null {
...
function requirePharmacist(req: AuthedRequest, res: Response, next: NextFunction): void {
...
function requirePatient(req: AuthedRequest, res: Response, next: NextFunction): void {
  const id = decodeBearerId(req.headers.authorization);
  if (!id || PHARMACIST_IDS.has(id)) {
