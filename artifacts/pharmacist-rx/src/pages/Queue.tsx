...
function ageBucketPill(days: number): { label: string; cls: string } {
  if (days <= 3) return { label: `${days}d ago`, cls: "bg-[#E7F4D5] text-[#0E3D2D]" };
  if (days <= 7) return { label: `${days}d ago`, cls: "bg-amber-100 text-amber-700" };
  return { label: `${days}d ago`, cls: "bg-red-100 text-red-700" };
}

function medicationFor(c: Consultation): { primary: string; tag: string; tagCls: string } {
...
export function Queue() {
  ...
  const rows = allRows.filter((c) => {
    if (search) {
      const hay = `${c.patientName} ${c.patientEmail} ${c.conditionName} ${c.id}`.toLowerCase();
    ...
    <div className="min-h-full">
      <div className="max-w-[1500px] mx-auto px-6 py-5">
        <div className="flex items-start justify-between gap-4">
          ...
          <button
            className="inline-flex items-center gap-2 px-3 py-2 text-sm rounded-md border border-stone-200 bg-white text-stone-700 hover:bg-stone-50"
        ...
        <div className="rounded-xl bg-amber-50 border border-amber-200 p-3 mt-4 flex items-center gap-3">
          <AlertTriangle className="h-5 w-5 text-amber-600 shrink-0" />
          <div className="flex-1 min-w-0">
            <div className="text-sm font-semibold text-amber-900">
              Chargeback risk monitoring — review orders approaching the 14-day dispute window.
            </div>
          </div>
          <button
            type="button"
            className="bg-amber-500 hover:bg-amber-600 text-white text-xs font-semibold rounded-full px-4 py-1.5 shrink-0"
          >
            Review now
          </button>
        </div>
        ...
        <Card className="p-4 mt-4">
          ...
          <div className="flex items-center gap-2 flex-wrap mt-3">
            {(
              ["Awaiting Review", "0-3 days", "3-7 days", "7+ days", "Auto-flagged", "Urgent"] as const
            ).map((label) => {
              ...
              return (
                <button
                  key={label}
                  type="button"
                  className="inline-flex items-center gap-1.5 px-3 py-1 text-xs rounded-full bg-stone-100 text-stone-700 hover:bg-stone-200"
                >
                  {label}
                  {count > 0 && (
                    <span className="inline-flex items-center justify-center text-[10px] font-semibold rounded-full bg-white text-stone-600 h-4 min-w-4 px-1">
                      {count}
                    </span>
                  )}
                </button>
          ...
          <div className="flex items-center gap-3 mt-4 pt-3 border-t border-stone-100 flex-wrap">
            ...
            <input
              ...
              className="px-3 py-1.5 text-xs rounded-md border border-stone-200 bg-white outline-none"
            />
            ...
            <input
              ...
              className="px-3 py-1.5 text-xs rounded-md border border-stone-200 bg-white outline-none"
            />
            <div className="ml-auto flex items-center gap-2 bg-stone-100 rounded-md px-3 py-1.5 min-w-[260px]">
      ...
      <div className="max-w-[1500px] mx-auto px-6 mt-2">
        <Card className="overflow-hidden p-0">
          <div className="grid grid-cols-[40px_110px_90px_1.4fr_110px_1.8fr_140px_120px_120px_110px] gap-3 px-4 py-3 bg-stone-50 border-b border-stone-200 text-[10px] uppercase tracking-wide font-semibold text-stone-500 sticky top-0 z-10">
            <div>
          ...
          {rows.map((c) => (
            <QueueRow key={c.id} c={c} onOpen={() => navigate(`/orders/${c.id}`)} />
          ))}
        </Card>
